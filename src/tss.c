...
void tss_nueva_tarea(unsigned int* code, posicion pos){
	...
	unsigned int i = tarea_gdt_libre();
        ...
        (unsigned short)    (unsigned int) tss_nueva, 	    		/*aca hay q ponerle (bien) &tss_inicial*/         /* base[0:15]   */
    ...
    tss_nueva->ss0 = GDT_POSICION_DATA_KERNEL; //Verdura  
    ...
    tss_nueva->eflags = 0x202; //VERDURITA;
    tss_nueva->es = GDT_POSICION_DATA_USER;
    tss_nueva->cs = GDT_POSICION_CODE_USER;
    tss_nueva->ss = GDT_POSICION_DATA_USER;
    tss_nueva->ds = GDT_POSICION_DATA_USER; 		//level 3 datos.;
    tss_nueva->fs = GDT_POSICION_DATA_USER;
    tss_nueva->gs = GDT_POSICION_DATA_USER;
